...
Infected pea aphids exhibited distinctly larger hue angles and hence greener colors (@tbl-ANOVA, @fig-Aphids). This result supports the researchers' hypothesis that infection may lead to color change. This idea is also supported via a multiple comparison test, as well as a pairwise Tukey comparison (@tbl-Contrast, @tbl-aphidMeans).
...
```{r}
#| label: tbl-Contrast
#| tbl-cap: "Planned linear comparisson of means" 
...
aphidCont <- tidy(contrast
           ...
           'L1: Control,uninfected vs infected '    = c( -1,  -1,  2 ),
            'L2: uninfected vs infected'       = c(  0,  -1,  1 )
...
```{r}
#| label: tbl-aphidMeans
#| tbl-cap: "Pairwise comparisons of group means: mean differences with Tukey adjusted P-values."

aphidMeans <- emmeans(aphidMod, specs = "Treatment")
